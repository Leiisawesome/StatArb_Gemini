"""Concrete Polygon-oriented ingestion utilities for the successor package."""
...
class PolygonPayloadNormalizer(EventNormalizer):
    """Normalize common Polygon quote and trade payload shapes into successor events."""
...
class InMemoryPolygonDataSource(MarketDataSource):
    """Historical and pseudo-live source backed by in-memory Polygon payloads."""
    ...
    def __init__(
        ...
        self._active_halts: dict[str, int] = {}
        self._resume_exclusion_until_ns: dict[str, int] = {}
        self._active_auctions: dict[str, int] = {}
        self._auction_resume_exclusion_until_ns: dict[str, int] = {}
        self._active_trade_indices: dict[str, int] = {}
    ...
    def load_historical(self, request: HistoricalBatchRequest) -> Iterable[MarketEvent]:
        ...
        for event in self._events:
            ...
            if isinstance(event, TradeEvent) and not request.include_trades:
                continue
            if datetime.fromtimestamp(event.timestamp_ns / 1_000_000_000.0, tz=timezone.utc).astimezone(_EASTERN).date() != request.trade_date:
                continue
            yield event
    ...
    def _payload_is_eligible(self, payload: dict[str, Any] | MarketEvent) -> bool:
        if isinstance(payload, (QuoteEvent, TradeEvent)):
            if self._in_halt_exclusion(payload.symbol, payload.timestamp_ns):
                return False
            return True
        if not isinstance(payload, dict):
            return False
        ...
        if event_type in {"t", "trade"} or "price" in payload or "p" in payload:
            ...
            if self.filter_config.exclude_late_prints and self._is_late_print(payload):
                return False
            conditions = self._trade_conditions(payload)
            if conditions & self.filter_config.exclude_trade_conditions:
                return False
        return True
    ...
    def _payload_is_eligible_without_correction_filters(self, payload: dict[str, Any] | MarketEvent) -> bool:
        ...
        if conditions & filtered_conditions:
            return False
        return True
    ...
    def _trade_correction_instruction(payload: dict[str, Any] | MarketEvent) -> tuple[str, str] | None:
        ...
        if action_text in {"cancel", "delete", "remove", "cancelled"}:
            action = "cancel"
        else:
            action = "replace"
        try:
    ...
    def _auction_signal(payload: dict[str, Any] | MarketEvent) -> tuple[str, str, int] | None:
        ...
        if not has_status_shape or "auction" not in status_text and not any(
            key in payload for key in ("auction", "auction_active", "auction_state")
        ):
            return None
    ...
    def _is_late_print(payload: dict[str, Any]) -> bool:
        for key in ("is_late", "late", "late_report"):
            ...
            if value in (True, 1, "1", "true", "True"):
                return True
        return "late" in InMemoryPolygonDataSource._trade_conditions(payload)
...
class PolygonWebSocketDataSource(MarketDataSource):
    """Streaming Polygon source with an injectable WebSocket connection factory."""
    ...
    def subscribe_live(self, request: LiveSubscriptionRequest) -> Iterable[MarketEvent]:
        connection = self.connection_factory(self.websocket_config.endpoint)
        self._authenticate(connection)
        self._subscribe(connection, request)
        self._reset_halt_state()

        for payload in self._iter_payloads(connection):
            halt_signal = self._halt_signal(payload)
            if halt_signal is not None:
                self._apply_halt_signal(halt_signal)
                continue
            ...
            if event is None:
                continue
            if event.symbol not in set(request.symbols):
                continue
            if isinstance(event, QuoteEvent) and not request.include_quotes:
                continue
            if isinstance(event, TradeEvent) and not request.include_trades:
    ...
    def _authenticate(self, connection: Any) -> None:
        api_key = self.websocket_config.api_key
        if api_key:
            connection.send(json.dumps({"action": "auth", "params": api_key}))

    def _subscribe(self, connection: Any, request: LiveSubscriptionRequest) -> None:
        subscriptions = list(self.websocket_config.raw_subscriptions)
        if not subscriptions:
            subscriptions.extend(self._subscriptions_for_request(request))
        if subscriptions:
            connection.send(json.dumps({"action": "subscribe", "params": ",".join(subscriptions)}))

    def _subscriptions_for_request(self, request: LiveSubscriptionRequest) -> list[str]:
    ...
    def _iter_payloads(self, connection: Any) -> Iterator[dict[str, Any]]:
        messages: Iterable[Any]
        if hasattr(connection, "__iter__"):
            messages = connection
        elif hasattr(connection, "recv"):
            messages = self._recv_iterator(connection)
        else:
            raise TypeError("connection must be iterable or implement recv()")
    ...
    @staticmethod
    def _recv_iterator(connection: Any) -> Iterator[Any]:
        while True:
            try:
    ...
    def _payload_is_eligible(self, payload: dict[str, Any] | MarketEvent) -> bool:
        return InMemoryPolygonDataSource._payload_is_eligible(self, payload)

    def _event_is_eligible(self, event: MarketEvent) -> bool:
        return InMemoryPolygonDataSource._event_is_eligible(self, event)

    def _in_exclusion_window(self, symbol: str, timestamp_ns: int) -> bool:
        return InMemoryPolygonDataSource._in_exclusion_window(self, symbol, timestamp_ns)

    def _in_halt_exclusion(self, symbol: str, timestamp_ns: int) -> bool:
        return InMemoryPolygonDataSource._in_halt_exclusion(self, symbol, timestamp_ns)

    def _in_active_auction_exclusion(self, symbol: str, timestamp_ns: int) -> bool:
        return InMemoryPolygonDataSource._in_active_auction_exclusion(self, symbol, timestamp_ns)

    def _apply_halt_signal(self, signal: tuple[str, str, int]) -> None:
        InMemoryPolygonDataSource._apply_halt_signal(self, signal)

    def _apply_auction_signal(self, signal: tuple[str, str, int]) -> None:
        InMemoryPolygonDataSource._apply_auction_signal(self, signal)

    def _reset_halt_state(self) -> None:
        InMemoryPolygonDataSource._reset_halt_state(self)

    def _in_auction_exclusion(self, timestamp_ns: int) -> bool:
        return InMemoryPolygonDataSource._in_auction_exclusion(self, timestamp_ns)

    @staticmethod
    def _halt_signal(payload: dict[str, Any] | MarketEvent) -> tuple[str, str, int] | None:
        return InMemoryPolygonDataSource._halt_signal(payload)

    @staticmethod
    def _auction_signal(payload: dict[str, Any] | MarketEvent) -> tuple[str, str, int] | None:
        return InMemoryPolygonDataSource._auction_signal(payload)

    @staticmethod
    def _is_correction(payload: dict[str, Any]) -> bool:
        return InMemoryPolygonDataSource._is_correction(payload)

    @staticmethod
    def _is_late_print(payload: dict[str, Any]) -> bool:
        return InMemoryPolygonDataSource._is_late_print(payload)

    @staticmethod
    def _trade_conditions(payload: dict[str, Any]) -> set[str]:
        return InMemoryPolygonDataSource._trade_conditions(payload)

    @staticmethod
    def _default_connection_factory(endpoint: str) -> Any:
        try:
            from websockets.sync.client import connect
        except ImportError as exc:
            raise RuntimeError("websockets sync client is required for PolygonWebSocketDataSource") from exc
