...
class DeterministicReplayEngine:
    def __init__(self):
        self._stream_id = "uninitialized"
        self._speed_multiplier = 1.0
        self._current_index = 0
        self._current_timestamp_ns = 0
    ...
    def replay(self, events: Iterable[MarketEvent], speed_multiplier: float = 1.0) -> Iterable[MarketEvent]:
        ...
        start_index = self._current_index if self._stream_id == stream_id else 0
        self._stream_id = stream_id
        self._speed_multiplier = speed_multiplier
        self._current_index = start_index
        self._current_timestamp_ns = (
            ordered_events[start_index - 1].timestamp_ns if ordered_events and start_index > 0 else 0
        )
        for index in range(start_index, len(ordered_events)):
            event = ordered_events[index]
            self._current_index = index + 1
            self._current_timestamp_ns = event.timestamp_ns
            yield event

    def checkpoint(self) -> ReplayCheckpoint:
        return ReplayCheckpoint(
            stream_id=self._stream_id,
            event_index=self._current_index,
            event_timestamp_ns=self._current_timestamp_ns,
        )

    def restore(self, checkpoint: ReplayCheckpoint) -> ReplayCursor:
        self._stream_id = checkpoint.stream_id
        self._current_index = checkpoint.event_index
        self._current_timestamp_ns = checkpoint.event_timestamp_ns
        return ReplayCursor(
