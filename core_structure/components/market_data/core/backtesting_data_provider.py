...
class BacktestingDataProvider:
    ...
    def __init__(self, clickhouse_loader, data_request):
        ...
        # Try to import production safety framework (optional)
        try:
            from core_structure.infrastructure.production_safety import (
            ...
            self.has_safety_framework = True
            ...
            self.has_safety_framework = False
            self.logger.info("Production safety framework not available, using basic validation")
        
    def _production_safe(self, failure_mode=None):
        """Decorator replacement when production safety framework is not available"""
        def decorator(func):
            return func
        return decorator
    ...
    def _update_current_prices(self, index: int):
        ...
        if self.historical_data is not None and not self.historical_data.empty and index < len(self.historical_data):
            ...
            for symbol in self.data_request.symbols:
                ...
                if not symbol_data:
                    # Handle missing symbol data
                    if self.has_safety_framework and hasattr(self, 'safety_framework'):
                        try:
                            from core_structure.infrastructure.production_safety import Environment, ValidationError
                            current_env = self.safety_framework.get_current_environment()
                            
                            if current_env.value in ['PRODUCTION', 'STAGING']:
                            ...
                            self.logger.warning(f"Missing symbol data for {symbol}, using defaults")
                            symbol_data = {'open': 100.0, 'high': 105.0, 'low': 95.0, 'close': 102.0}
                    else:
                        # No safety framework, use defaults
                        self.logger.warning(f"Missing symbol data for {symbol}, using defaults")
                        symbol_data = {'open': 100.0, 'high': 105.0, 'low': 95.0, 'close': 102.0}
    ...
    def get_current_price(self, symbol: str) -> Optional[float]:
        ...
        # Handle missing data according to environment
        if self.has_safety_framework and hasattr(self, 'safety_framework'):
            try:
                from core_structure.infrastructure.production_safety import Environment
                current_env = self.safety_framework.get_current_environment()
                ...
                # Fallback if imports fail
                return self.symbol_prices.get(symbol, 100.0)
        else:
            # No safety framework, use fallback
            return self.symbol_prices.get(symbol, 100.0)
    ...
    def get_execution_price(self, symbol: str, side: str) -> Optional[float]:
        ...
        if not isinstance(symbol_data, dict):
            # Handle missing OHLC data according to environment
            if self.has_safety_framework and hasattr(self, 'safety_framework'):
                try:
                    from core_structure.infrastructure.production_safety import Environment
                    current_env = self.safety_framework.get_current_environment()
