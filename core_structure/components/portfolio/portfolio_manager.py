...
class Position:
    ...
    def calculate_total_pnl(self, current_price: float) -> float:
        """Calculate total P&L (realized + unrealized) at current price"""
...
class PortfolioManager(IRegimeSubscriber if REGIME_ENGINE_AVAILABLE else object):
    ...
    def add_position(self, position: Position):
        ...
        self._update_portfolio_totals()
        logger.info(f"Added position: {position.symbol} {position.quantity} @ ${position.avg_price:.2f}")
    ...
    def update_position(self, symbol: str, quantity: int, price: float, trade_type: str):
        """Update position with a trade"""
        # Handle negative quantities - convert to positive and adjust trade type
        if quantity < 0:
            quantity = abs(quantity)
            trade_type = "SELL"
        
        # Use the existing process_trade method
        self.process_trade(symbol, quantity, price, trade_type)
    
    @property
    def portfolio_value(self) -> float:
        """Get total portfolio value"""
        return self.total_market_value + self.available_capital
    ...
    def calculate_portfolio_metrics(self, current_prices: Dict[str, float]) -> PortfolioMetrics:
        ...
        return self.get_portfolio_metrics(current_prices)
    
    def get_single_position_metrics(self, symbol: str, current_prices: Dict[str, float]) -> Optional[PositionMetrics]:
        """Get metrics for a specific position"""
        position = self.get_position(symbol)
        if position and symbol in current_prices:
            return position.get_metrics(current_prices[symbol], self.portfolio_value)
        return None
    
    def get_all_position_metrics(self, current_prices: Dict[str, float]) -> Dict[str, PositionMetrics]:
        """Get metrics for all positions"""
        return self.get_position_metrics(current_prices)
    ...
    def from_dict(self, data: Dict):
        ...
        self.total_unrealized_pnl = data.get('total_unrealized_pnl', 0.0)
        ...
        for symbol, pos_data in data.get('positions', {}).items():
            ...
            position.total_pnl = pos_data.get('total_pnl', 0.0)
            position.market_value = pos_data.get('market_value', 0.0)
            
            if 'created_at' in pos_data:
                position.created_at = datetime.fromisoformat(pos_data['created_at'])
            if 'updated_at' in pos_data:
                position.updated_at = datetime.fromisoformat(pos_data['updated_at'])
            
            self.positions[symbol] = position
        
        logger.info(f"Loaded portfolio from dict: {len(self.positions)} positions")
