#!/usr/bin/env python3
"""
Factor Strategy Assessment
Test the Enhanced Factor Strategy on 1-minute data
"""
...
async def test_factor_config(tester, config_name, strategy_config, base_params):
    """Test a specific Factor configuration"""
    ...
    test_config = StrategyTestConfig(
        strategy_name="Enhanced Factor Strategy",
...
async def main():
    """Run comprehensive Factor strategy tests"""
    
    logger.info("="*80)
    logger.info("🚀 Starting Factor Strategy Assessment")
    logger.info("="*80)
    
    # Initialize tester
    output_dir = "tests/strategy_assessment/results/factor"
    ...
    # Test 1: BASELINE - Multi-factor model
    baseline_config = {
        'factors': ['momentum', 'value', 'quality', 'volatility'],  # Factor types
        'rebalance_frequency': 5,      # Rebalance every 5 periods
        'factor_lookback': 60,         # 60 periods for factor calculation
        'base_position_pct': 0.03,     # 3% base position size
        'max_position_pct': 0.09,      # 9% maximum position size
    }
    results['baseline'] = await test_factor_config(
        tester, 'BASELINE', baseline_config, base_params
    )
    
    # Test 2: CONSERVATIVE - Lower risk, higher thresholds
    conservative_config = {
        'factors': ['momentum', 'value'],  # Fewer factors for stability
        'rebalance_frequency': 10,     # Less frequent rebalancing
        'factor_lookback': 80,         # Longer lookback for stability
        'base_position_pct': 0.02,     # Smaller base position size
        'max_position_pct': 0.06,      # Lower maximum position size
    }
    results['conservative'] = await test_factor_config(
        tester, 'CONSERVATIVE', conservative_config, base_params
    )
    
    # Test 3: AGGRESSIVE - Higher risk, more signals
    aggressive_config = {
        'factors': ['momentum', 'value', 'quality', 'volatility', 'size'],  # More factors
        'rebalance_frequency': 3,      # More frequent rebalancing
        'factor_lookback': 40,         # Shorter lookback for responsiveness
        'base_position_pct': 0.04,     # Larger base position size
        'max_position_pct': 0.12,      # Higher maximum position size
    }
    results['aggressive'] = await test_factor_config(
        tester, 'AGGRESSIVE', aggressive_config, base_params
    )
    
    logger.info(f"\n{'='*80}")
    logger.info("📊 FACTOR STRATEGY COMPARISON")
    logger.info(f"{'='*80}\n")
    ...
    logger.info(f"\n{'='*80}")
    logger.info("✅ Factor Strategy Assessment Complete!")
    logger.info(f"{'='*80}\n")
