...
class EnhancedMeanReversionStrategy(EnhancedBaseStrategy):
    ...
    def _evaluate_signal_conditions(
        ...
        try:
            ...
            # ========================================
            # INDEPENDENT STOP-LOSS CHECK (PRIORITY #1)
            # ========================================
            # Check stop-loss BEFORE any other logic - this ensures we exit
            # even when z-score is in the wrong direction (oversold during drop)
            has_position_details = hasattr(self, '_position_details') and self._position_details
            if has_position_details:
                pos_info = self._position_details.get(symbol)
                if pos_info and pos_info.get('quantity', 0) > 0:
                    pnl_pct = pos_info.get('pnl_pct', 0)
                    stop_loss_pct = getattr(self.config, 'stop_loss_pct', -5.0)
                    take_profit_pct = getattr(self.config, 'take_profit_pct', 10.0)
                    
                    # FORCE EXIT if stop-loss hit (regardless of z-score direction!)
                    if pnl_pct <= stop_loss_pct:
                        ...
                        cooldown_bars = getattr(self.config, 'stop_loss_cooldown_bars', 30)
                        if not hasattr(self, '_stop_loss_cooldown'):
                            self._stop_loss_cooldown = {}
                        self._stop_loss_cooldown[symbol] = idx + cooldown_bars
                        
                        # Increment stop-loss counter
                        if not hasattr(self, '_stop_loss_count'):
                            self._stop_loss_count = {}
                        self._stop_loss_count[symbol] = self._stop_loss_count.get(symbol, 0) + 1
                        
                        logger.warning(f"[{symbol}] 🛑 STOP-LOSS TRIGGERED: pnl={pnl_pct:+.2f}% <= {stop_loss_pct}%, "
                                      f"entry=${entry_price_pos:.2f}, current=${current_price:.2f}, "
                                      f"cooldown until bar {idx + cooldown_bars}, count={self._stop_loss_count[symbol]}")
                        
                        # Get timestamp and price for signal
                        timestamp = current_row.get('timestamp', datetime.now()) if isinstance(current_row, pd.Series) else datetime.now()
                        exit_price = current_row['close'] if isinstance(current_row, pd.Series) else current_row.get('close', 0)
                        
                        # Create FORCED SELL signal
                        return StrategySignal(
                            symbol=symbol,
                            signal_type=SignalType.SELL,
                            strength=1.0,  # Maximum urgency
                            confidence=0.95,  # High confidence - must exit
                            timestamp=timestamp,
                    ...
                    # FORCE EXIT if take-profit hit (regardless of z-score direction!)
                    elif pnl_pct >= take_profit_pct:
                        entry_price_pos = pos_info.get('entry_price', 0)
                        current_price = pos_info.get('current_price', 0)
                        logger.info(f"[{symbol}] 🎯 TAKE-PROFIT TRIGGERED: pnl={pnl_pct:+.2f}% >= {take_profit_pct}%, "
                                   f"entry=${entry_price_pos:.2f}, current=${current_price:.2f}")
                        ...
                        return StrategySignal(
                            ...
                            signal_type=SignalType.SELL,
                            strength=1.0,  # Maximum urgency
                            confidence=0.99,  # Very high confidence - lock in profit
                            timestamp=timestamp,
                            entry_price=exit_price,
                            additional_data={
                                'signal_reason': 'take_profit_triggered',
                                'pnl_pct': pnl_pct,
            ...
            if is_entry_signal:
                ...
                # ========================================
                # Don't re-enter for N bars after a stop-loss exit
                if not hasattr(self, '_stop_loss_cooldown'):
                    self._stop_loss_cooldown = {}  # {symbol: cooldown_until_bar_index}
                
                cooldown_bars = getattr(self.config, 'stop_loss_cooldown_bars', 30)  # 30 bars = 30 min
                cooldown_until = self._stop_loss_cooldown.get(symbol, 0)
                
                if idx < cooldown_until:
                    logger.debug(f"[{symbol}] ❄️ STOP-LOSS COOLDOWN: waiting until bar {cooldown_until} (current: {idx})")
                    return None  # Still in cooldown
                
                # ========================================
                # TREND FILTER (DOWNTREND PROTECTION)
                # ========================================
                # Don't buy if price is clearly in a downtrend (below SMA_50)
                sma_50 = self._get_indicator_value(data, 'sma_50', idx, default=None)
                sma_20 = self._get_indicator_value(data, 'sma_20', idx, default=None)
                current_price = current_row['close'] if isinstance(current_row, pd.Series) else current_row.get('close', 0)
                
                if sma_50 is not None and current_price < sma_50 * 0.98:  # Price 2%+ below SMA_50
                    # Additional check: Is SMA_20 also below SMA_50? (death cross = strong downtrend)
                    if sma_20 is not None and sma_20 < sma_50:
                        logger.debug(f"[{symbol}] 📉 TREND FILTER BLOCK: price=${current_price:.2f} < SMA50=${sma_50:.2f}, "
                                    f"SMA20=${sma_20:.2f} < SMA50 (death cross) - not buying in downtrend")
                        return None  # Don't buy in strong downtrend
                    else:
                        logger.debug(f"[{symbol}] ⚠️ TREND WARNING: price below SMA50 but no death cross")
                
                # ========================================
                # MAX CONSECUTIVE STOP-LOSS CHECK
                # ========================================
                # Stop trading if we've hit too many stop-losses recently
                if not hasattr(self, '_stop_loss_count'):
                    self._stop_loss_count = {}  # {symbol: count_today}
                
                max_stop_losses = getattr(self.config, 'max_consecutive_stop_losses', 2)
                stop_loss_count = self._stop_loss_count.get(symbol, 0)
                
                if stop_loss_count >= max_stop_losses:
                    logger.warning(f"[{symbol}] 🚫 MAX STOP-LOSSES REACHED: {stop_loss_count} >= {max_stop_losses} - "
                                  f"stopping trading on this symbol for the day")
                    return None  # Stop trading this symbol
